...
if (
    ...
    print
"USAGE : drawBlastVsSeqRef.pl -input_file FAM000015_no_match_part.gff3 -type gff -id FAM15 -ref_length 6917 -round 250 -sort start
    
Options : 
    -input_file      File to draw (string, blast = resume.csv)      (Mandatory)
    -type            Type of the file : blast or gff                (Mandatory)
    -id              Name of the reference (string)                 (Mandatory if type = gff)
    -ref_length      Length of the reference (string)               (Mandatory if type = gff)
...
if ($config{verbose} and $config{type} eq "gff") {
    print "id : \t\t$config{id}\n";
...
if ($config{type} eq "blast") {
    while(<FILE>){
        ...
        my @tmp = split(/;/, $_);
        if ($tmp[9] > -1){
            $data{$tmp[2]}{'start'} .= $tmp[11]."\t";
            $data{$tmp[2]}{'end'}   .= $tmp[12]."\t";
            $data{$tmp[2]}{'length'} = $tmp[10];
            if (exists($data{$tmp[2]})) {$data{$tmp[2]}{'nbseq'}++;}
            else                        {$data{$tmp[2]}{'nbseq'} = 1;}
        }
    }
}
...
$data{$id}{'length'} = $length;
    while(<FILE>){
        ...
        $data{$id}{'end'}   .= $2."\t";
        $data{$id}{'nbseq'}++;
        #if (exists($data{$id})) {$data{$id}{'nbseq'}++;}
        #else                    {$data{$id}{'nbseq'} = 1;}
    }
}
close FILE;
print "OK ! \n" if $config{verbose};
...
foreach my $cons (keys(%data)){
    
    print "Seq Ref : $cons\n" if $config{verbose};
    
    my @start = split(/\t/, $data{$cons}{'start'});
    my @end = split(/\t/, $data{$cons}{'end'});
...
#    my $epaisseur = 10;
    ...
    my $largeur = $data{$cons}{'length'} + 2 * $margeL;
    my $hauteur = 2 * $epaisseur * $data{$cons}{'nbseq'} + 2 * $margeH + 2 * 2 * $epaisseur;
    ...
    $im->filledRectangle(   $curseurL, $curseurH,
                            $curseurL + $data{$cons}{'length'}, $curseurH + $epaisseur,
                            $black);
